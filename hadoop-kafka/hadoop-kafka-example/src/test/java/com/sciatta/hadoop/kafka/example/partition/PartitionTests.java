package com.sciatta.hadoop.kafka.example.partition;
...
public class PartitionTests {
    ...
    public void testTo1Partition() {
        producer = UtilFactory.getKafkaProducer();
        // 向1号分区发送数据
        producer.send(new ProducerRecord<>(UtilFactory.TOPIC_TEST, 1, String.valueOf(1), String.valueOf(1)));
        producer.close();
    }
    ...
    public void testToInvalidPartition() {
        producer = UtilFactory.getKafkaProducer();
        // 2号是无效分区
        producer.send(new ProducerRecord<>(UtilFactory.TOPIC_TEST, 2, String.valueOf(2), String.valueOf(2)));
        producer.close();
    }
